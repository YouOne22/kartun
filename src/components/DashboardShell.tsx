    ...
    <div className="flex min-h-screen bg-[#F8FAFC] text-[#0F172A]">
      ...
      <div className="flex min-w-0 flex-1 flex-col md:pl-[270px]">
        <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-4 border-b border-slate-200 bg-white px-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-3"><button aria-label="Buka menu" onClick={() => setOpen(true)} className="rounded-lg p-2 text-slate-600 hover:bg-slate-100 md:hidden"><Menu size={20} /></button><div className="flex min-w-0 items-center gap-3"><Image src="/logo.png" alt="Logo" width={48} height={48} className="h-12 w-12 shrink-0 object-contain" /><div className="min-w-0"><p className="truncate text-sm font-bold tracking-tight sm:text-base">TUNAS HARAPAN</p><p className="text-[11px] text-slate-500">Dusun Kemitir</p></div></div><h1 className="sr-only">{title}</h1></div>
          ...
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            ...
            <div className="space-y-6 p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div><p className="text-xs font-medium text-slate-400">Seksi / Jabatan Pengurus</p><p className="mt-0.5 text-sm font-semibold text-slate-800">{user.section || "-"}</p></div>
              </div>
