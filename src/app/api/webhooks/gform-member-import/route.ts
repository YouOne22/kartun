...
export async function POST(request: Request) {
  ...
  try {
    ...
    const user = await prisma.user.create({ data: { memberId: `KT-TH-${year}-${String(count + 1).padStart(3, "0")}`, fullName, email, passwordHash: await hash(process.env.DEFAULT_PASSWORD || "Dusun2026", 12), isDefaultPassword: true, gender: body.gender === "P" ? "P" : "L", birthPlace: safeText(body.birth_place, 50) || null, birthDate, phoneWa, rt: safeText(body.rt, 5) || "-", rw: safeText(body.rw, 5) || "-", address: safeText(body.address, 2000) || null, education: safeText(body.education, 30) || null, occupation: safeText(body.occupation, 50) || null, qrCodeToken: randomUUID() } });
