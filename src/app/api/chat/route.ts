...
export async function GET(request: Request) {
  ...
  try {
    const messages = await prisma.chatMessage.findMany({ where: { eventId }, include: { sender: { select: { id: true, fullName: true, memberId: true, role: true, avatarUrl: true } } }, orderBy: { createdAt: "asc" }, take: 200 });
...
export async function POST(request: Request) {
  ...
  try {
    const chatMessage = await prisma.chatMessage.create({ data: { message, eventId: rawEventId || null, senderId: auth.session.userId }, include: { sender: { select: { id: true, fullName: true, memberId: true, role: true, avatarUrl: true } } } });
